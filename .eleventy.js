...
module.exports = function (eleventyConfig) {
  ...
  // IndexNow key file (Bing, Yandex, Naver): must be served at the site root.
  eleventyConfig.addPassthroughCopy("c125bccae38be4eb0e8ba482cff3b8b0.txt");

  // Self-hosted brand webfonts (fonts/*.woff2 + fonts/fonts.css), generated by
  // scripts/fetch_google_fonts.py. Served from our own origin so no request
  // reaches fonts.gstatic.com — see partials/head-fonts.njk.
  ...
  // Chart.js for /comcenter, self-hosted instead of fetched from jsdelivr
  // so the page opens no third-party connection.
  eleventyConfig.addPassthroughCopy("chart.umd.js");
  ...
  // ── Trailing-slash transform ──────────────────────────────────────
  // vercel.json sets trailingSlash: false, so every internal link that ends
  // with "/" (research hubs, crosslinks, nav) cost a 308 redirect hop for
  // crawlers and users. Normalise internal hrefs to the slash-less form at
  // build time; external URLs (http...) and the root "/" are untouched.
  // Registered after the cross-link transform so injected .x-term links are
  // normalised too.
  eleventyConfig.addTransform("stripTrailingSlash", function (content, outputPath) {
    if (!outputPath || !outputPath.endsWith(".html")) return content;
